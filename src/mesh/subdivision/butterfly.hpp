...
namespace Mesh::Subdivision {
	...
	class Butterfly  final : public Mesh::Subdivision::Polymorphic {
	...
	public:
		...
		std::unique_ptr<Mesh::HalfEdge> process(
			...
			if ( p_input == nullptr ) {
				std::cout << "Error! Butterfly algorithm can not process a null pointer.\n";
				return nullptr;
			}
			...
			for ( auto const& polygon : p_input->polygon ) {
				...
				auto const v1 = mesh->add_vertex( polygon->edge[0]->vertex->location );
				auto const v2 = mesh->add_vertex( polygon->edge[1]->vertex->location );
				auto const v3 = mesh->add_vertex( polygon->edge[2]->vertex->location );
				// Edge boundary state
	...
	private:
		...
		Double3 CalculateEdgePoint(
			...
			// u ( p1 + p2 ), u = 0.5
			Double3 q = ( edge->vertex->location + edge->next->vertex->location ) * .5;
			...
			// v ( p3 + p4 ), v = 2 w = 1/8
			q += Corner( edge ) / 8.; // p3 from E1
			q += Corner( edge->twin ) / 8.; // p4 from E4

			// -w ( p5 + p6 + p7 + p8 ), w = 1 / 16
			if ( edge->next->twin ) // E2->twin
				q -= Corner( edge->next->twin ) / 16.; // p7
			if ( edge->next->next->twin ) // E3->twin
				q -= Corner( edge->next->next->twin ) / 16.; // p5
			if ( edge->twin->next->twin ) // E5->twin
				q -= Corner( edge->twin->next->twin ) / 16.; // p6
			if ( edge->twin->next->next->twin ) // E6->twin
				q -= Corner( edge->twin->next->next->twin ) / 16.; // p8
