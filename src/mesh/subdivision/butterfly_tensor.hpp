...
namespace Mesh::Subdivision {
	...
	public:
		...
		std::unique_ptr<Mesh::HalfEdge> process(
			...
			if ( p_input == nullptr ) {
				std::cout << "Error! ButterflyTensor can not process a null pointer.\n";
				return nullptr;
			}
			...
			for ( auto const& polygon : p_input->polygon ) {
				if ( polygon->edge.size() != 3 ) {
					std::cout << "Error! ButterflyTensor algorithm can only process a triangle mesh.\n";
					return nullptr;
				}
				for ( auto const& edge : polygon->edge )
					if ( edge->next == nullptr || edge->previous == nullptr) {
						std::cout << "Error! ButterflyTensor algorithm can only process closed loop polygons.\n";
						return nullptr;
					}
			}
