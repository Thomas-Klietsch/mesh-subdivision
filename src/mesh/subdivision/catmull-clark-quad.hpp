...
namespace Mesh::Subdivision {
	...
	public:
		...
		std::unique_ptr<Mesh::HalfEdge> process(
			...
			if ( p_input == nullptr ) {
				std::cout << "Error! Catmull-Clark-Quad algorithm can not process a null pointer.\n";
				return nullptr;
			}

			for ( auto const& polygon : p_input->polygon ) {
				if ( polygon->edge.size() != 4 ) {
				...
				for ( auto const& edge : polygon->edge ) {
					if ( edge->next == nullptr || edge->previous == nullptr ) {
						std::cout << "Error! Catmull-Clark-Quad algorithm can only process closed loop polygons.\n";
						return nullptr;
					}
			...
			for ( auto const& vertex : p_input->vertex )
				if ( vertex.use_count() != 5 ) {
					std::cout << "Error! Catmull-Clark-Quad algorithm need vertices with four (4) edges.\n";
					return nullptr;
				}
	...
	private:
		...
		Double3 CalculateVertex(
			...
			// This corner
			Double3 const p11 = edge->vertex->location;
			// Next corner
			Double3 const p12 = edge->next->vertex->location;
			// Opposite corner
			Double3 const p22 = edge->next->next->vertex->location;
			// Previous corner
			Double3 const p21 = edge->previous->vertex->location;
